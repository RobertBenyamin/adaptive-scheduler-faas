...
def build_features_from_history(history):
    ...
    Features capture statistical properties and trends.
    """
    if len(history) < 5:
        return None
    ...
    for i in range(window_size, len(history)):
        ...
        features = np.array([
            ...
            np.std(window) if len(window) > 1 else 0,  # Volatility
            np.max(window),               # Peak execution time
            np.min(window),               # Minimum execution time
            window[-1],                   # Most recent execution time
...
def train_xgb_model(history):
    """
    Train XGBoost model on execution history.
    Uses infrequent training to minimize overhead.
    """
    ...
    try:
        ...
        model = xgb.XGBRegressor(
            n_estimators=50,        # Number of trees (conservative)
...
def predict_xgb(model, history):
    """
    Use XGBoost model to predict next execution time.
    """
...
def calculate_remaining_time(pid):
    ...
    Falls back to v8 logic (EWMA) when XGBoost is unavailable.
    """
    global xgb_model
    ...
    # --- Try XGBoost Prediction (only if we have enough data) ---
    if len(history) >= 10:  # Minimum viable data
        # Train infrequently (every 15 new samples or when model is None)
        with xgb_model_lock:
